#!/usr/bin/env bash
set -euo pipefail
cd "$(dirname "$0")"  # Immer im serverfiles-Verzeichnis arbeiten
...
for tool in curl jq unzip; do
  if ! command -v "$tool" &>/dev/null; then
    echo -e "${RED}[ERROR] '$tool' ist nicht installiert. Bitte installiere es zuerst (apt install curl jq unzip).${RESET}"
    exit 2
  fi
done
...
download_server_pack() {
  ...
  # Neueste File-ID abrufen, falls "latest"
...
prepare_server() {
  ...
  # startserver.sh ausführen, falls vorhanden (Forge/Fabric-Installer)
